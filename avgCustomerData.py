...
for customer in data:
    for key in customer.keys():
        if isinstance(customer[key], str):
            ...
            avg[key] = avg.get(key,{"min":99999999,"max":-99999999,"avg":0,"amt":0})
            ...
            for key2 in customer[key].keys():
                if isinstance(customer[key][key2], str):
                    continue
                elif isinstance(customer[key][key2], float) or isinstance(customer[key2], int):
                    avg[key] = avg.get(key,{})
                    avg[key][key2] = avg[key].get(key2,{"min":99999999,"max":-99999999,"avg":0,"amt":0})
                    avg[key][key2]["min"] = min(avg[key][key2]["min"],customer[key][key2])
                    avg[key][key2]["max"] = max(avg[key][key2]["max"],customer[key][key2])
                    avg[key][key2]["amt"] +=1
