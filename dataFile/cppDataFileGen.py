...
EffectNamesStr = (
    "{" + ",".join([f'\"{effect}\"' for effect in EffectNames]) + "}"
)
...
BaseBaseEffectsStr = "{"+",".join([formatAsEffectSet(effect) for effect in BaseBaseEffects])+"}"
...
ItemPricesStr = "{"+",".join([str(int(price*floatScaler)) for price in ItemPrices])+"}"
ItemEffectsStr =  "{"+",".join([str(effect) for effect in ItemEffects])+"}"
ItemEffectReplaceMaskStr = "{"+",".join([formatAsEffectSet(mask) for mask in ItemEffectReplaceMask])+"}"
